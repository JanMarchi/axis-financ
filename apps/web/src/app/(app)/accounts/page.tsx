...
export default function AccountsPage() {
  const accounts = [
    { id: '1', name: 'Conta Corrente', type: 'checking', balance: 5230.50, source: 'manual' },
    { id: '2', name: 'Poupança', type: 'savings', balance: 8000.00, source: 'manual' },
    { id: '3', name: 'Cartão de Crédito', type: 'credit_card', balance: 3450.75, source: 'manual' },
  ];
  ...
  return (
    <div>
      <PageHeader
        title="Minhas Contas"
        description="Gerencias suas contas bancárias e cartões"
        action={{
        ...
        {accounts.map((account) => (
          <div key={account.id} className="bg-bg-elevated border border-border-subtle rounded-card p-6">
            ...
            <div className="flex items-center justify-between">
              <StatusBadge status={account.source === 'manual' ? 'active' : 'pending'} label={account.source === 'manual' ? 'Manual' : 'Automático'} size="sm" />
